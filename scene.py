...
class SquareAndCircle(Scene):
    def construct(self):
        ...
        square = Square()  # create a square
        square.set_fill(DARK_BLUE, opacity=0.5)  # set the color and transparency
...
class DifferentRotations(Scene):
    def construct(self):
        left_square = Square(color=DARK_BLUE, fill_opacity=0.7).shift(2 * LEFT)
...
#             NumberPlane(x_range=[-10,0,10], x_length=10, y_range=[-10,0,10],y_length=5)
#             .to_edge(DOWN)
#             .add_coordinates()
#         )

#         parab = plane.get_graph(lambda x: x ** 2, x_range=[-10,10], color=PINK)
